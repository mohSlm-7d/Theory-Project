...
void main() {
    ...
    while (*bufferPtr != '\0' && !Stack.empty()) {
        ...
        CheckRowAndCol(Stack.top(), bufferPtr[loc], Row, Col);
        ...
        //char* charPtr = &ProductionRule[];
        for (int i = ProductionRule.length() - 1; i>=2; i--) {
            Stack.push(ProductionRule[i]);
        }


        if (Stack.top() == bufferPtr[loc] && Stack.top() >= 'a' && Stack.top() <= 'z') {
            Stack.pop();
            loc++;
            cout << endl << "case 0";
        }
       
        if (ProductionRule[2]=='#' && bufferPtr[loc] != 'a') {
           
            /*cout << "Printing" << endl;
           for (int i = 0; bufferPtr[i] != '\0'; i++)
               cout << bufferPtr[i];*/
           
           
           Stack.pop();
           //cout << endl << endl << "ParsingTable[Row][Col]: " << ParsingTable[Row][Col];

           //cout << endl << "Stack After: " << endl;
               //cout << Stack.top() << endl;

               //cout << "Printing" << endl;
               /*for (int i = loc; bufferPtr[i] != '\0'; i++)
                   cout << bufferPtr[i];*/
        ...
        else if (Stack.top() == '$' && bufferPtr[loc] == '\0') {
            cout << endl << endl << "Accepted (Valid)!";
            cout << endl << "case 2";
        }
        
        else if (Stack.top() >= 'a' && Stack.top() <= 'z' && bufferPtr[loc] != Stack.top()) {
            cout << endl << endl << "Invalid.";
            cout << endl << "case 3";
        }
    }

    if (Stack.empty() && bufferPtr[loc] != '\0') {
        cout << endl << endl << "Invalid.";
    }
