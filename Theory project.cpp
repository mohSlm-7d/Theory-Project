...
int main() {
  ...
  std::printf("\n\nEnter you'r string here to validate it (Note Maxinmum length is "
    ...
    while (*bufferPtr != '\0' && !Stack.empty()) {
        ...
        if (IsVT(*bufferPtr, VT)  && IsVT(Stack.top(), VT) && *bufferPtr == Stack.top() ) {
            Stack.pop();
            bufferPtr++;
        }

         else if (IsVT(*bufferPtr, VT)  && IsVT(Stack.top(), VT) && *bufferPtr != Stack.top() ) {
            flag= 0;
            break;
        }

         else if(IsVN(Stack.top(), VN) && IsVT(*bufferPtr, VT)){
